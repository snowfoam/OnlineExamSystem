			...
			<div class="test">
				<form action="" method="post">
					<div class="test_title">
						...
						<font><input type="button" onclick="submitExam()" name="test_jiaojuan" value="Submit"></font>
					</div>
						...
						<div class="test_content">
							<div class="test_content_title">
								<h2>Single Choice</h2>
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.singleQuestionNum }</i><span>&nbsp;question，</span><span>total&nbsp;</span><i class="content_fs">${singleScore * exam.singleQuestionNum}</i><span>&nbsp;score</span>
								</p>
							</div>
						</div>
								...
								<c:forEach items="${singleQuestionList}" var="sq" varStatus="sqids">
									<li id="qu_${singleQuestion }_${sq.question.id}" data-key="${sq.id}">
										<div class="test_content_nr_tt">
											<i>${sqids.index+1}</i><span>(${sq.question.score}score)</span><font>${sq.question.title }</font>
										</div>
						...
						<div class="test_content">
							<div class="test_content_title">
								...
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.muiltQuestionNum }</i><span>&nbsp;question，</span><span>total&nbsp;</span><i class="content_fs">${exam.muiltQuestionNum * muiltScore}</i><span>&nbsp;score</span>
								</p>
							</div>
						</div>
						<div class="test_content_nr">
							<ul>
								<c:forEach items="${muiltQuestionList}" var="mq" varStatus="mqids">
									<li id="qu_${muiltQuestion }_${mq.question.id}" data-key="${mq.id}">
										<div class="test_content_nr_tt">
											<i>${mqids.index + exam.singleQuestionNum + 1 }</i><span>(${mq.question.score}score)</span><font>${mq.question.title }</font>
										</div>
					...
					<c:if test="${exam.chargeQuestionNum !=0 }">
						<div class="test_content">
							<div class="test_content_title">
								<h2>Judgment</h2>
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.chargeQuestionNum }</i><span>&nbsp;question，</span><span>total&nbsp;</span><i class="content_fs">${chargeScore * exam.chargeQuestionNum}</i><span>&nbsp;score</span>
								</p>
							</div>
						</div>
						<div class="test_content_nr">
							<ul>
								<c:forEach items="${chargeQuestionList}" var="cq" varStatus="cqids">
									<li id="qu_${chargeQuestion }_${cq.question.id}" data-key="${cq.id}">
										<div class="test_content_nr_tt">
											<i>${cqids.index+exam.singleQuestionNum+exam.muiltQuestionNum+1}</i><span>(${cq.question.score}score)</span><font>${cq.question.title }</font>
										</div>
			...
			<div class="nr_rt_main">
				<div class="rt_nr1">
					<div class="rt_nr1_title">
						<h1>
							<span style="font-size:18px;">Answer Sheet</span>
						</h1>
						<p class="test_time">
					...
					<c:if test="${exam.singleQuestionNum !=0 }">
						<div class="rt_content">
							<div class="rt_content_tt">
								<h2>Single Choose</h2>
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.singleQuestionNum }</i><span>&nbsp;question</span>
								</p>
							</div>
						...
						<div class="rt_content">
							<div class="rt_content_tt">
								<h2>Multiple Choice</h2>
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.muiltQuestionNum }</i><span>&nbsp;question</span>
								</p>
							</div>
							<div class="rt_content_nr answerSheet">
								<ul>
					...
					<c:if test="${exam.chargeQuestionNum !=0 }">
						<div class="rt_content">
							<div class="rt_content_tt">
								<h2>Judgment</h2>
								<p>
									<span>total&nbsp;</span><i class="content_lit">${exam.chargeQuestionNum }</i><span>&nbsp;question</span>
								</p>
							</div>
							<div class="rt_content_nr answerSheet">
								<ul>
...
function autoSubmitExam(){
	...
	$.ajax({
		...
		success: function(data){
			if(data.type == 'success'){
				alert('The test time is up and the test results are out. Please check in the member center');
...
function submitExam(){
	var wcm = window.confirm('No more exams after submission, confirm submission？');
	if(!wcm){
		return;
	}
	//提交试卷
	$.ajax({
		...
		success: function(data){
			if(data.type == 'success'){
				alert('Exam results have been issued, please go to the member center to view');
				window.location="../user/index";
			}else{
				alert(data.msg);
				//$(".tm_btn_primary").text('提交');
