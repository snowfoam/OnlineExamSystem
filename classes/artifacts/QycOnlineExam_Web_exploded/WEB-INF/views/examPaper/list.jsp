	...
	<form id="edit-form" method="post">
        ...
        <table>
            <tr>
                ...
                <td>
                	<select id="edit-examId" name="examId" class="easyui-combobox easyui-validatebox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'Please select test subjects'">
		                <c:forEach items="${examList}" var="exam">
			            	<option value="${exam.id}">${exam.name}</option>
		            	</c:forEach>
		            </select>
            ...
            <tr>
                ...
                <td>
                	<select id="edit-studentId" name="studentId" class="easyui-combobox easyui-validatebox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'Please select test subjects'">
		                <c:forEach items="${studentList}" var="student">
			            	<option value="${student.id}">${student.name}</option>
		            	</c:forEach>
		            </select>
	...
	*/
	function openEdit(){
		...
		$('#edit-dialog').dialog({
			closed: false,
			modal:true,
            title: "Edit test paper information",
            buttons: [{
                text: 'determine',
                iconCls: 'icon-ok',
                handler:edit
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			...
			{ field:'status',title:'Exam Status',width:200,formatter:function(value,index,row){
				if(value == 0) return 'not tested';
				return 'tested';
			}},
			{ field:'startExamTime',title:'Exam Start Time',width:200,formatter:function(value,index,row){
				if(value == '' || value == null)return '';
				return format(value);
			}},
			{ field:'endExamTime',title:'Exam End Time',width:200,formatter:function(value,index,row){
				if(value == '' || value == null)return '';
				return format(value);
			}},
			{ field:'useTime',title:'Exam time',width:200,formatter:function(value,index,row){
				return value + 'minute';
			}},
			{ field:'totalScore',title:'Total Score',width:200},
			{ field:'score',title:'Exam Score',width:200},
